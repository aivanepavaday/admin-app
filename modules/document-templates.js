...
export function renderDocCard(doc, expandedIds = new Set()) {
  ...
  /* Aperçu */
  const previewInner = doc.thumbnail
    ? `<img class="doc-thumb" src="${doc.thumbnail}" alt="" loading="lazy">`
    : `<div class="doc-thumb-placeholder"><span class="doc-ext-lg">${escHtml(fileExt(doc.name))}</span></div>`;
  ...
  const trashSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/></svg>`;
  const chevron  = `<svg class="expand-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>`;

  return `
    <div class="doc-card${isExpanded ? ' expanded' : ''}" data-id="${doc.id}" style="--cat-color:${color}">
      <div class="doc-card-preview">
        ${previewInner}
        <button class="doc-view-btn" type="button" title="Ouvrir le document">${eyeSvg}Voir</button>
      </div>
      ...
      <button class="doc-card-expand-toggle" type="button" title="${isExpanded ? 'Réduire' : 'Voir les détails'}">
        <span>${isExpanded ? 'Réduire' : 'Détails'}</span>
        ${chevron}
      </button>
      <div class="doc-card-details">${detailsInner}</div>
      <div class="doc-card-actions-expanded">
        <button class="doc-view-btn-full"  type="button">${eyeSvg}Voir</button>
        <button class="doc-card-edit"      type="button">${editSvg}Modifier</button>
        <button class="doc-card-remove"    type="button">${trashSvg}Supprimer</button>
      </div>
    </div>`;
}
